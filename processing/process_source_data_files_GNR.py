...
def read_process_pv_df():
    ...
    df_pv = df_pv[["date", "SAP", "Prix_GNR_TTC"]].rename(
        columns={"SAP": "sap", "Prix_GNR_TTC": "prix"}
    )
    df_pv["site"] = "Carrefour"

    df_pv = df_pv.merge(df_depot[["code", "sap", "depot"]], on="sap", how="inner")
...
def read_process_concu_df():
    df_concu = pd.read_pickle("../data/source/data_concu_gnr.pkl").drop("ZT", axis=1)
    df_concu = df_concu.merge(
        df_depot[["code", "sap", "depot"]], on=["sap", "code"], how="left"
    )
    df_concu = df_concu[["date", "code", "sap", "depot", "site", "prix"]]
    df_concu["depot"] = df_concu["depot"].apply(str.strip)
...
def read_process_depot_df():
    df_depot = pd.read_pickle("../data/source/data_depot_gnr.pkl")
    df_depot.loc[df_depot["sap"] == "DDPL", "depot"] = "DP LYON"
    return df_depot
...
df_prices = df_prices.merge(df_regions[["depot", "region"]], on="depot", how="left")
